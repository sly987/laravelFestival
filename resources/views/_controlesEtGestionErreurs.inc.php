...
function verifierDonneesGrC($connexion, $idGroupe, $nomGroupe, $nombrePersonnes, $nomPays )
{
   ...
   {
      ajouterErreur ("Le nombre de personne doit être un entier");
   }
}
function verifierDonneesGroupeM($connexion, $idGroupe, $nomGroupe, $nombrePersonnes, $ligue, $hebergement)
{
   if ($nomGroupe=="" || $nombrePersonnes=="" || $ligue == "" || $hebergement = "")
   {
      ajouterErreur("Chaque champ suivi du caractère * est obligatoire");
   }
   if ($nomGroupe!="" && estUnNomGroupe($connexion, 'M', $idGroupe, $nomGroupe))
   {
      ajouterErreur("L'établissement $nom existe déjà");
   }
   if ($nombrePersonnes!="" && (!estEntier($nombrePersonnes))|| $nombrePersonnes<0)
   {
      ajouterErreur("Le nombre de personne est non entière ou inférieure à 0");
   }
}
